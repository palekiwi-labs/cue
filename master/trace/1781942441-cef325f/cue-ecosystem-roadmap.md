...
session. Not a plan — a sequenced set of phases with ordering rationale for
use as a prioritisation reference when creating kanban tasks.

Revised after identifying that a stateless `acuity` MVP (session.idle ->
Gotify) is a real production need that also serves as the honest proof of the
cross-repo schema contract — making it a better Phase 1 than the original
curator-first ordering.
...
| 1     | `acuity` stateless MVP: session.idle -> Gotify | Cross-repo schema contract, proven end-to-end; replaces production server |
